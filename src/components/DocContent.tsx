...
FM Mailには実装の都合上、以下の制限がございます。あらかじめご了承ください。
...
エラーが発生すると、データは取り込まれていないのにIMAP4サーバー上のメールが既読状態となる場合があります。「新着メール受信」では既読状態のメールは取り込まれません。メールの取りこぼしが発生した場合は「UID指定で受信」を用いて再取り込みしてください。

### APIの回数制限

1か月あたりのAPIの実行回数には上限があります。実行回数が上限に達するとAPIを呼び出してもエラーとなります。FREE版の上限は100回です。メールの受信頻度を上げたい場合はPRO版をご検討ください。
