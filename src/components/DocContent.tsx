...
上部メニューの「APIキーの確認」をクリックしてください。APIキーが表示されるのでコピーしてください。
...
APIの接続先URL|次の値を入力してください。\`https://obosev6836.execute-api.ap-northeast-1.amazonaws.com/api/fetch_mail\`
...
FM Mailには実装の制約上、以下の制限がございます。あらかじめご了承ください。
...
システムをクラウド上に構築している関係上、一過性のエラーが発生する場合があります。その場合はしばらく時間をおいてから再実行してください。

APIがエラーになると、受信は失敗したのにメールサーバー上のメールは既読状態になる場合があります。「新着メール受信」では既読状態のメールは取り込まれないため、メールの取りこぼしが発生した場合は「UID指定で受信」を用いて再取り込みしてください。

### APIの回数制限

一月あたりのAPIの実行回数には上限があります。実行回数が上限に達するとAPIを呼び出してもエラーとなります。FREE版の上限は100回です。それよりメールの受信頻度を上げたい場合はPRO版をご検討ください。
`;
